...
public class QuestMsgController extends    BaseController {
    ...
    @Override
    public void pushPluginAction(PushPluginAction mPPA, int requestCode) {
        resultCode=0;
        if (ValueUtils.isNotEmpty(mPPA)){

            //分发控制器
            if (mPPA.getServiceType().equals(ConstServiceType.MSG_SERVICE)){
                resultPluginAction(mPPA.getPluginId(),requestCode, resultCode, new ProtocolMsgBean("0000", "操作成功", mPPA.getData(),requestCode,resultCode));

            }else if (mPPA.getServiceType().equals(ConstServiceType.INFRASTRUCTURE_SERVICE)){
